...
def run_evaluation():
    ...
    print("=" * 70)
    print("RAG EVALUATION SYSTEM - Day 31")
    print("=" * 70)
    print()
    ...
    for item in questions:
        ...
        try:
            ...
            print(f"  [STEP 1] RAG responded. status={ok}")
            print(f"  [STEP 1] response retrieved: {message}")

            # STEP 2: Check if answer matches expected
            print(f"  [STEP 2] Checking answer quality...")
            answer_match = check_answer_match(message, expected_keyword)
            ...
            test_passed = answer_match
            if test_passed:
                passed += 1
                print(f"  [RESULT] ✓ PASS (answer={answer_match})")
            else:
                failed += 1
                reasons = []
